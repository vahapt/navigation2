...
class LifecycleTransitionTestNode : public nav2_ros_common::LifecycleNode
{
public:
  explicit LifecycleTransitionTestNode(rclcpp::NodeOptions options)
  : nav2_ros_common::LifecycleNode("test_node", "", options) {}

  nav2_ros_common::CallbackReturn on_configure(const rclcpp_lifecycle::State &) override
  {
    configured = true;
    return nav2_ros_common::CallbackReturn::SUCCESS;
  }
  nav2_ros_common::CallbackReturn on_activate(const rclcpp_lifecycle::State &) override
  {
    activated = true;
    return nav2_ros_common::CallbackReturn::SUCCESS;
  }
...
TEST(LifecycleNode, RclcppNodeExitsCleanly)
{
  ...
  auto node1 = std::make_shared<nav2_ros_common::LifecycleNode>("test_node", "");
...
TEST(LifecycleNode, MultipleRclcppNodesExitCleanly)
{
  ...
  auto node1 = std::make_shared<nav2_ros_common::LifecycleNode>("test_node1", "");
  auto node2 = std::make_shared<nav2_ros_common::LifecycleNode>("test_node2", "");
...
TEST(LifecycleNode, OnPreshutdownCbFires)
{
  ...
  class MyNodeType : public nav2_ros_common::LifecycleNode
  {
public:
    MyNodeType(
      const std::string & node_name)
    : nav2_ros_common::LifecycleNode(node_name) {}
...
protected:
    void on_rcl_preshutdown() override
    {
      ...
      nav2_ros_common::LifecycleNode::on_rcl_preshutdown();
    }
  };
